<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<html>
<head>
	 <script type="text/javascript" src="js/rsa/Barrett.js"></script>
	 <script type="text/javascript" src="js/rsa/BigInt.js"></script>
	 <script type="text/javascript" src="js/rsa/RSA.js"></script>
	  <script type="text/javascript" src="js/jquery.js"></script>
	 <script type="text/javascript" src="js/titanpay.js"></script>
</head>
<body>
<h2>Hello World!</h2>
<button value="asdfadfasd" id="payButton" >pay</button>
</body>
<script type="text/javascript">
	$(function(){
		
		
	 var pub = "";
	 var expo = "10001";
		
	 initTitanPay('127.0.0.1:8082', expo, pub);
		
	 //GDP 支付采用固定的账户编码
	 
//name n, escrowedDate n, goodsId y, goodsdetail n, goodsname n, userId y, ruserId n, amount y, paytype 
	  var orderInfo = {
			name:"",   //打开收银台人员姓名  N
			escrowedDate:"",//保证期时间  N
			goodsId:"",//商品编号，可以是对方的订单号
			goodsDetail:"",//商品描述  N
			goodsName:"",//商品名称 N
			userId:"",//付款方身份标示   如果是财务，则建议是FCUSERID，  如果是GDP，则是用户ID
			ruserId:"",//收款方身份标示 N  ,GDP可以指定接受方的   商家联盟可以指定其FCUSERID
			amount:"",//订单金额
			payerType:"",//付款人类型   财务 GDP 等
			notify:'http://www.baidu.com'
		};

		var businessInfo=
		{
			inAccountCode:"11111",
			outAccountCode:"53245234532"
		}

		$("#payButton").click(function() {

			titanPay(orderInfo, businessInfo);
		});

	
	});
</script>
</html>
