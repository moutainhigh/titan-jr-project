...
public class TitanLoanServiceListenerImpl implements TitanLoanServiceListener {
	...
	private static final Log log = LogFactory.getLog(TitanLoanServiceListenerImpl.class);
	...
	public boolean  loanSucceed(String buessNo, int state) {
		...
		LoanOrderNotifyResponse loanOrderNotifyResponse = loanService.loanOrderNotify(loanOrderNotifyRequest);
		if(!loanOrderNotifyResponse.isResult()){
			log.error("贷款申请通知处理失败,rspmsg:"+loanOrderNotifyResponse.getReturnMessage()+",泰坦金融贷款订单号buessNo:"+buessNo+",state:"+state);
			return false;
		}
		return true;
	}
