...
$('.sendmes').on('click',function(){
	...
	if(flag == true){
		if(!$(this).hasClass("huise")){
	    	$.ajax({
	    		 type: "post",
		         url: "<%=basePath%>/account/sendCode.action",
		         data: {
		        	 receiveAddress:phoneOrEmail
		         },
		         dataType: "json",
...
function validate_email_or_phone(data){//验证其输入的是不是邮箱或者手机号
	if(data.length>0){
		...
		if(email.test(data) || phone.test(data) ){
			return true;
		}
		return false;
	}
...
var PasswordStr=new sixDigitPassword("passwordbox3");
var PasswordStr1=new sixDigitPassword("passwordbox4");
