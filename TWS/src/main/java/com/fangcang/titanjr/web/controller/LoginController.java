...
public class LoginController extends BaseController{
    ...
    public String sendCode(String receiveAddress,Integer msgType){
    	...
    	if(msgType==SMSType.REG_CODE.getType()){//注册
    		sendCodeRequest.setContent("尊敬的用户： 您正在申请开通泰坦金融服务，验证码为："+regCode+"，验证码"+WebConstant.REG_CODE_TIME_OUT_HOUR+"小时内有效。如不是您申请，请勿将验证码发给其他人。");
    		sendCodeRequest.setSubject("泰坦金融注册验证码");
    	}else if(msgType==SMSType.PAY_PASSWORD_MODIFY.getType()){//修改付款密码
    		sendCodeRequest.setContent("尊敬的用户： 您正在修改泰坦金融的付款密码，验证码为："+regCode+"，验证码"+WebConstant.REG_CODE_TIME_OUT_HOUR+"小时内有效。请勿将验证码发给其他人。");
    		sendCodeRequest.setSubject("泰坦金融修改付款密码");
    	}else if(msgType==SMSType.LOGIN_PASSWORD_MODIFY.getType()){//修改登录密码
    		sendCodeRequest.setContent("尊敬的用户： 您正在修改泰坦金融的登录密码，验证码为："+regCode+"，验证码"+WebConstant.REG_CODE_TIME_OUT_HOUR+"小时内有效。请勿将验证码发给其他人。");
    		sendCodeRequest.setSubject("泰坦金融修改登录密码");
    	}else if(msgType==SMSType.LOGIN_BY_CODE.getType()){//动态码登录
    		sendCodeRequest.setContent("尊敬的用户： 您正在使用动态验证码登录泰坦金融，验证码为："+regCode+"，验证码"+WebConstant.REG_CODE_TIME_OUT_HOUR+"小时内有效。请勿将验证码发给其他人。");
    		sendCodeRequest.setSubject("动态验证码登录");
    	}
    	
    	SendMessageResponse sendMessageResponse = sendSMSService.sendMessage(sendCodeRequest);
    	if(sendMessageResponse.isResult()){
    		return toJson(putSuccess("验证码发送成功"));
    	}else{
    		return toJson(putSysError(sendMessageResponse.getReturnMessage()));
    	}
